...
class InvertedResidualBlock(nn.Module):
    def __init__(self, inp, oup, expand_ratio):
        ...
        self.conv2 = nn.Conv2d(hidden_dim, hidden_dim, 3, groups=hidden_dim, bias=False)
        self.conv2_ = nn.Conv2d(hidden_dim, hidden_dim, 1, groups=hidden_dim, bias=False)
...
class DetailNode(nn.Module):
    def __init__(self, dim):
        ...
        dim_half = dim//2
        self.theta_phi = InvertedResidualBlock(inp=dim_half, oup=dim_half, expand_ratio=2)
        self.theta_rho = InvertedResidualBlock(inp=dim_half, oup=dim_half, expand_ratio=2)
        self.theta_eta = InvertedResidualBlock(inp=dim_half, oup=dim_half, expand_ratio=2)
...
class DetailFeatureExtractionNetwork_2(nn.Module):
    def __init__(self, dim, num_blocks=2):
        super(DetailFeatureExtractionNetwork_2, self).__init__()
        layers = []
        for _ in range(num_blocks):
            layers.append(AsymmetricConvBlock(dim, dim))
        self.network = nn.Sequential(*layers)

    def forward(self, x):
        return self.network(x)

class DetailFeatureExtractionNetwork_3(nn.Module):
    def __init__(self, dim, num_blocks=2):
        super(DetailFeatureExtractionNetwork_3, self).__init__()
        layers = []
        self.pre_conv = nn.Conv2d(dim, dim//2, kernel_size=1)
        
        for _ in range(num_blocks):
            layers.append(AsymmetricConvBlock(dim//2, dim//2))
        self.post_conv = nn.Conv2d(dim//2, dim, kernel_size=1)
        self.network = nn.Sequential(*layers)
        self.activate = nn.SiLU(inplace=True)
    ...
    def forward(self, x):
        x = self.pre_conv(x)
        x = self.network(x)
        x = self.activate(self.post_conv(x))
