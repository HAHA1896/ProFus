...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels):
        ...
        for i in range(3):
            ...
            radar = labels_patch["radar"]
            rvm = labels_patch["rvm"]
            h, w = labels_patch.pop("resized_shape")

            # Place img in img3
            if i == 0:  # center
                img3 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                radar3 = np.full((s * 3, s * 3, radar.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                rvm3 = np.full((s * 3, s * 3, rvm.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                h0, w0 = h, w
            ...
            radar3[y1:y2, x1:x2] = radar[y1 - padh :, x1 - padw :]  # img3[ymin:ymax, xmin:xmax]
            rvm3[y1:y2, x1:x2] = rvm[y1 - padh :, x1 - padw :]  # img3[ymin:ymax, xmin:xmax]
        ...
        final_labels["radar"] = radar3[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        final_labels["rvm"] = rvm3[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                ...
                radar4 = np.full((s * 2, s * 2, radar.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                rvm4 = np.full((s * 2, s * 2, rvm.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            ...
            radar4[y1a:y2a, x1a:x2a] = radar[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
            rvm4[y1a:y2a, x1a:x2a] = rvm[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
            padw = x1a - x1b
            padh = y1a - y1b
        ...
        final_labels["radar"] = radar4
        final_labels["rvm"] = rvm4
        return final_labels
    ...
    def _mosaic9(self, labels):
        ...
        for i in range(9):
            ...
            if i == 0:  # center
                ...
                radar9 = np.full((s * 3, s * 3, radar.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                rvm9 = np.full((s * 3, s * 3, rvm.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
            ...
            radar9[y1:y2, x1:x2] = radar[y1 - padh :, x1 - padw :]  # img9[ymin:ymax, xmin:xmax]
            rvm9[y1:y2, x1:x2] = rvm[y1 - padh :, x1 - padw :]  # img9[ymin:ymax, xmin:xmax]
            hp, wp = h, w  # height, width previous for next iteration
        ...
        final_labels["radar"] = radar9[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        final_labels["rvm"] = rvm9[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        final_labels = {
            ...
            "radar_file": mosaic_labels[0]["radar_file"],
            "rvm_file": mosaic_labels[0]["rvm_file"],
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels["radar"] = (labels["radar"] * r + labels2["radar"] * (1 - r)).astype(np.uint8)
        labels["rvm"] = (labels["rvm"] * r + labels2["rvm"] * (1 - r)).astype(np.uint8)
...
class RandomPerspective:
    ...
    def affine_transform(self, img, radar, rvm, border):
        """
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ...
                radar = cv2.warpPerspective(radar, M, dsize=self.size, borderValue=(114, 114, 114))
                rvm = cv2.warpPerspective(rvm, M, dsize=self.size, borderValue=(114, 114, 114))
            else:  # affine
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                radar = cv2.warpAffine(radar, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                rvm = cv2.warpAffine(rvm, M[:2], dsize=self.size, borderValue=(114, 114, 114))
        return img, radar, rvm, M, s
    ...
    def __call__(self, labels):
        ...
        img, radar, rvm, M, scale = self.affine_transform(img, radar, rvm, border)
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            hue, sat, val = cv2.split(cv2.cvtColor(rvm, cv2.COLOR_BGR2HSV))
            im_hsv = cv2.merge((cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat), cv2.LUT(val, lut_val)))
            cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR, dst=rvm)  # no return needed
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        labels["radar"] = np.ascontiguousarray(radar)
        labels["rvm"] = np.ascontiguousarray(rvm)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None, radar=None, rvm=None):
        """Return updated labels and image with added border."""
        if labels is None:
            labels = {}
        img = labels.get("img") if image is None else image
        radar = labels.get("radar") if radar is None else radar
        rvm = labels.get("rvm") if rvm is None else rvm
        ...
        if shape[::-1] != new_unpad:  # resize
            ...
            radar = cv2.resize(radar, new_unpad, interpolation=cv2.INTER_LINEAR)
            rvm = cv2.resize(rvm, new_unpad, interpolation=cv2.INTER_LINEAR)
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        img = cv2.copyMakeBorder(
            img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
        radar = cv2.copyMakeBorder(
            radar, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
        rvm = cv2.copyMakeBorder(
            rvm, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            rd_new = np.zeros(rd.shape, np.uint8)
            rv_new = np.zeros(rv.shape, np.uint8)
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                ...
                cv2.drawContours(rd_new, instances.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
                cv2.drawContours(rv_new, instances.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)

            result = cv2.flip(im, 1)  # augment segments (flip left-right)
            result_rd = cv2.flip(rd, 1)  # augment segments (flip left-right)
            result_rv = cv2.flip(rv, 1)  # augment segments (flip left-right)
            i = cv2.flip(im_new, 1).astype(bool)
            i_rd = cv2.flip(rd_new, 1).astype(bool)
            i_rv = cv2.flip(rv_new, 1).astype(bool)
            im[i] = result[i]
            rd[i_rd] = result_rd[i_rd]
            rv[i_rv] = result_rv[i_rv]
    ...
    def __call__(self, labels):
        ...
        if len(cls):
            ...
            if self.transform and random.random() < self.p:
                ...
                new_rv = self.transform(image=rv, bboxes=bboxes, class_labels=cls)  # transformed
                if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                    labels["img"] = new["image"]
                    labels["rvm"] = new_rv["image"]
...
class Format:
    ...
    def __call__(self, labels):
        ...
        labels["img"], labels["radar"], labels["rvm"] = self._format_img(img, radar, rvm)
    ...
    def _format_img(self, img, radar, rvm):
        """Format the image for YOLO from Numpy array to PyTorch tensor."""
        if len(img.shape) < 3:
            img = np.expand_dims(img, -1)
        img = img.transpose(2, 0, 1)
        radar = radar.transpose(2, 0, 1)
        rvm = rvm.transpose(2, 0, 1)

        if random.uniform(0, 1) > self.bgr:
            img = np.ascontiguousarray(img[::-1])
            radar = np.ascontiguousarray(radar[::-1])
            rvm = np.ascontiguousarray(rvm[::-1])

        img = torch.from_numpy(img)
        radar = torch.from_numpy(radar)
        rvm = torch.from_numpy(rvm)
