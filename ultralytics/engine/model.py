...
class Model(nn.Module):
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False) -> None:
        ...
        # print("engine->model.model:_new.cfg_dict:", cfg_dict)
        self.cfg = cfg
        # print("task before guess:", task)
        self.task = task or guess_model_task(cfg_dict)
        # print("task guess:", self.task)
        self.model = (model or self._smart_load("model"))(cfg_dict, verbose=verbose and RANK == -1)  # build model
        # print("创建新模型：", self.model)
    ...
    def load(self, weights: Union[str, Path] = "yolov8n.pt") -> "Model":
        ...
        Raises:
            AssertionError: If the model is not a PyTorch model.
        """
        print("加载预训练权重：", weights)
        self._check_is_pytorch_model()
        if isinstance(weights, (str, Path)):
            weights, self.ckpt = attempt_load_one_weight(weights)
        # print("输入模型的权重：", weights.keys())
        with open("预训练模型的key_1_before.txt", "w") as file:
        ...
        #     # 可以根据需要添加更多的替换规则
        # }
        # 遍历 state_dict 的键，逐个替换
        state_dict = weights.float().state_dict()
        new_state_dict = {}
        for key in state_dict.keys():
            ...
            new_key = key
            # print(key)
            # for old_suffix, new_suffix in replace_map.items():
            #     if old_suffix in new_key:
            #         new_key = new_key.replace(old_suffix, new_suffix)
            # if "model.16" in new_key:
            #     break
            new_state_dict[new_key] = state_dict[key]
        new_state_dict["model.37.dfl.conv.weight"] = state_dict["model.23.dfl.conv.weight"]
        with open("预训练模型的key_1_after.txt", "w") as file:
            for key in new_state_dict.keys():
                file.write(key + "\n")

        # initial_weights = self.model.model[0].conv.weight.clone()
        self.model.load(new_state_dict, first_init=True)
